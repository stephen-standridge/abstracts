//creates a 2-dimensional grid of x by y points evenly spaced .
// at its core, it is just an array with constant time access of any point on the grid.
// all iteration happens in linear time because of this.
...
class Grid {
	...
	iterate(callback) {
		...
		for (i = 0; i < this.dimensions[1]; i++) {
			...
			for (j = 0; j < this.dimensions[2]; j++) {
				z = this.dimensions[2] - 1 == 0 ? 0 : j / (this.dimensions[2] - 1);
				returned.push(callback([i, j], [x, z]))
			}
		}
		return returned
	}
