...
import { BoundingBox } from '../../../source/space/bounds';
...
describe('BoundingBox', () => {
	...
	describe('#size', () => {
		it('should calculate the sizes of the bounding box', () => {
			bounding_box = new BoundingBox([-2, 0, 3], [4, -2, 2])
			expect(bounding_box.size()).to.have.members([6, 2, 1])
		})
		describe('with a dimension', () => {
			before(() => {
				bounding_box = new BoundingBox([-2, 1], [-5, 5])
			})
			it('should return the size for the given dimension', () => {
				expect(bounding_box.size(1)).to.equal(4)
			})
			it('should be undefined for greater dimensions', () => {
				expect(bounding_box.size(2)).to.equal(undefined)
			})
		})
	})
	describe('#extent', () => {
